export function validate(user,setError) {
    ...
    if(!user?.password) tempError.passErr = "password is required";
    else if(!/^[A-Za-z0-9]{6,9}$/.test(user?.password)) tempError.passErr = "Invalid password. min 6 char and max 9 characters.";
...
// login 
export function loginValidate(user,setError) {
    let tempError = {}
